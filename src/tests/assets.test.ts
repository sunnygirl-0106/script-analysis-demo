...
// **不要为了让测试通过去放宽阈值。**
//
// v1.2：着装角色（look）是关系资产，imagePrompt 为 demo 机械拼接占位（决策 8.2），
// 不参与「资产提示词完全版」的严格校验。四类基础资产（BASE）才是被严格校验的对象。
...
const LOOKS = ALL.filter((a): a is Look => a.kind === 'look')
const BASE = ALL.filter((a) => a.kind !== 'look') // 四类基础资产：严格校验对象
...
const REQUIRED: Record<Exclude<Asset['kind'], 'look'>, string[]> = {
...
describe('资产提示词完整性', () => {
  it('19 个基础资产全部就位（角色 4 · 服装 4 · 场景 4 · 道具 7）', () => {
    const byKind = (k: Asset['kind']) => BASE.filter((a) => a.kind === k).length
  ...
  it('每个角色都至少有一个着装角色（look）—— 挂到镜头上的是穿好衣服的定妆照', () => {
    for (const ch of BASE.filter((a) => a.kind === 'character')) {
      const own = LOOKS.filter((l) => l.characterId === ch.id)
      expect(own.length, `角色「${ch.name}」没有任何着装角色`).toBeGreaterThanOrEqual(1)
    }
  })
  ...
  it('四类基础资产的段式各自齐全', () => {
    for (const a of BASE) {
      ...
      for (const tag of REQUIRED[a.kind as Exclude<Asset['kind'], 'look'>]) {
  ...
  it('角色素模里不得混进戏服（素模 + 服装分开生成再融合）', () => {
    ...
    for (const a of BASE.filter((x): x is Character => x.kind === 'character')) {
